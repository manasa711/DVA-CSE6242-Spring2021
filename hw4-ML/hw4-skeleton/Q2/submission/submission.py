...
class Utility(object):
    ...
    def entropy(self, class_y):
        ...
        unique_tup = np.unique(class_y, return_counts=True)
        count = unique_tup[1]

        probability_vals = count/len(class_y)

        n_class = np.count_nonzero(probability_vals)
        if n_class <= 1:
            return 0

        for i in probability_vals:
            log_val = i*log(i,2)
            entropy = entropy - log_val
    ...
    def information_gain(self, previous_y, current_y):
        ...
        for i in current_y:
            calc = self.entropy(i)*len(i)/len(previous_y)
            info_gain = info_gain - calc
    ...
    def best_split(self, X, y):
       ...
       'y_right': [0, 0, 1]
        ...
        for i in range(len(X[0])):
            arr = np.asarray(X)[:,i]
            mean_arr = np.mean([m for m in arr])

            temp_split_val = np.asarray(mean_arr)

            partition = self.partition_classes(X,y, i, temp_split_val)
            ...
            if info_gain > current_gain:
                current_gain = info_gain
                split_val = temp_split_val
                split_attribute = i

        final_partition = self.partition_classes(X, y, split_attribute, split_val)
        X_left = final_partition[0]
        X_right = final_partition[1]
        y_left = final_partition[2]
        y_right = final_partition[3]

        d_vals = {'split_attribute':split_attribute, 'split_val':split_val, 'X_left':X_left, 'X_right':X_right, 'y_left':y_left, 'y_right':y_right, 'info_gain':info_gain}
...
class DecisionTree(Utility):
    def __init__(self, max_depth):
        ...
        self.tree = {"split_attribute": -1,"split_val":0,"left_node":np.nan,"right_node":np.nan}
    ...
    def build_tree(self,X,y,depth =0):
        ...
        if (depth > self.max_depth) or (len(set(y)) == 1) or (y_length <= 3):
            return {"split_attribute": -1,"split_val": np.mean(y),"left_node":np.nan,"right_node":np.nan}

        best_split_dict = self.best_split(X, y)
        split_attribute = best_split_dict['split_attribute']
        split_value = best_split_dict['split_val']
        X_left = best_split_dict['X_left']
        X_right = best_split_dict['X_right']
        y_left = best_split_dict['y_left']
        y_right = best_split_dict['y_right']

        left_node = self.build_tree(X_left,y_left,depth +1)
        right_node = self.build_tree(X_right,y_right,depth +1)

        d_vals = {"split_attribute": split_attribute,"split_val": split_value,"left_node": left_node,"right_node": right_node}
    ...
    def node_classification(self, record,node):
        ...
        if record[split_attribute] >= split_value:
            return self.node_classification(record,right_node)
        else:
            return self.node_classification(record,left_node)
    ...
    def classify(self, record):
        ...
        return self.node_classification(record,self.tree)
        #############################################
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                var = -1
                dataset_len = len(self.bootstraps_datasets)

                for i in range(dataset_len):
                    var = self.bootstraps_datasets[i].index(record)
                    if var != -1:
                        y = np.append(y, self.bootstraps_labels[i][var])
                        break
